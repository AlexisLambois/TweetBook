...
exemples de paramétrage des fichiers context.xml et web.xml.

Pour mettre en oeuvre cet exemple il faut tout d'abord avoir une base
et creer les tables nécessaires. Ensuite, il faut paramétrer le
contexte (META-INF/context.xml à destination de server.xml),
Vérifier/corriger dans ce fichier les références physiques à la
BDD. Le web.xml est déjà pret à l'usage, il n'y a a priori rien à
toucher dedans, jetez y quand même un coup d'oeil rapide.
...
Le Realm utilise l'interface JDBCRealm. Dans votre base, creez les
tables permettant d'accéder aux (login,role,mdp) :
...
faudrait alors modifier les définitions du Realm dans context.xml

Ne pas oublier de mettre le driver postgresql.jar soit dans le repertoire
tomcat/lib soit vide_evolue/lib.
...
Bien vérifier dans ce fichier que toute référence physique externe est
bien paramétrée. C'est notamment le cas des références à la BDD dans
le Pool et dans le Realm.
